...
| --- | --- | --- | --- | --- |
| Demo | `munaxa/munaxademo` | `munaxademo` | Workers | OpenNext (`@opennextjs/cloudflare`) |
| Landing | `munaxa/landing` | `landing` | Workers | OpenNext (`@opennextjs/cloudflare`) |
| Design-system site | `munaxa/munaxadesignsystem` | `designsystem` | Pages | Vite static build |
...
> one level down under `munaxa/`. The apps' own paths changed; the dashboard settings below did
...
| **Root directory** | `/` (the **repository root** — *not* `munaxa/` and *not* the app subfolder) |
| **Build command** | `pnpm --filter <app> run cf:build` |
| **Deploy command** | `pnpm --filter <app> exec wrangler deploy` |

Replace `<app>` with `munaxademo` or `@munaxa/landing`.
...
# Landing
pnpm --filter @munaxa/landing run cf:build
pnpm --filter @munaxa/landing run cf:preview
```
