...
Apply PERMISSION_ARCHITECTURE.md before rendering. Components assume safe, scoped data. Hidden controls do not enforce security. Sensitive counts, tags, autocomplete, notifications, and exports follow the same scope. Finance, safeguarding, health, and staff data require field-level capabilities.

## Workflow integration

Status props map to server-owned states documented in WORKFLOW_ARCHITECTURE.md. Components render current state and permitted actions but do not invent transitions. Mutations include record version, audit reason where required, idempotency key where applicable, and recoverable conflict handling.

## Notification integration

Domain events drive the notification policy in NOTIFICATION_ARCHITECTURE.md. Components may show durable state or current-action feedback, but channel selection, priority, deduplication, consent, and escalation remain policy services.
