...
2. **The live application** — `apps/admin/` (Next.js 15 + Tailwind v3) — which consumes **none** of the reference system. It uses a much thinner, independently-authored token preset (`@munaxa/config-tailwind`), a utility-only UI package (`@munaxa/ui`, which exports only `cn()`), and **8 hand-rolled local UI primitives**.
...
├── packages/
│   ├── ui/             ← @munaxa/ui  — currently exports ONLY cn()  ← key gap
│   ├── config-tailwind/← @munaxa/config-tailwind — the tokens the app ACTUALLY uses
│   ├── i18n/           ← @munaxa/i18n — EN + AR (641 keys each), RTL aware
...
### 4.1 Tokens the app actually uses — `@munaxa/config-tailwind/preset.ts` + `apps/admin/src/app/globals.css`
...
- **i18n:** `@munaxa/i18n` EN+AR (641 keys), `useI18n().t()`, localStorage `munaxa.locale`.
...
| Area | Gap |
|---|---|
| **`@munaxa/ui` is empty** | Exports only `cn()`; **zero shared components.** There is no consumable component layer bridging the reference system to the app |
...
| Tokens home | TS objects in DS | `@munaxa/config-tailwind` | Port missing scales into the preset |
| Components home | DS `components/ui` (53) | local 8 + empty `@munaxa/ui` | Build/port into `@munaxa/ui` |
| Domain components | DS `school/` (40+) | ~none | Port into `@munaxa/ui` (or new pkg) |
...
- **Phase 2 — Token Migration:** port missing reference scales into `@munaxa/config-tailwind`; confirm no hardcoded values remain.
- **Phase 3 — Core Components:** establish the canonical component layer in `@munaxa/ui` and migrate app primitives onto it (preserving APIs/behavior).
