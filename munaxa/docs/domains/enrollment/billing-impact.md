...
- **People:** Student/Parent/`ParentStudent`/Section/Grade CRUD; sibling detection (`billing.repository.siblingsOf`). **Transport:** `Bus*`/`StudentBusAssignment` = **logistics/GPS only** (no fee). **Structure:** `AcademicYear`/`Semester`/`Grade`. **DS UI:** `@munaxa/ui` + domain components (RecordHeader, ParentProfileDialog, status badges) + `Timeline`, `EmptyState`.
