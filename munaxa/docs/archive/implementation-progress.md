...
# 0. Prerequisites
pnpm install
pnpm build --filter=@munaxa/domain --filter=@munaxa/ui --filter=@munaxa/i18n
...
pnpm --filter @munaxa/api exec prisma migrate deploy --schema=../../prisma/schema.prisma

# 2. Confirm schema and ledger agree (must report no drift / no pending migration)
pnpm --filter @munaxa/api exec prisma migrate status --schema=../../prisma/schema.prisma
...
pnpm --filter @munaxa/api exec jest --config ./test/jest-e2e.json attendance-evolution

# 7. Regression: the pre-existing attendance/HR suites must stay green
pnpm --filter @munaxa/api exec jest --config ./test/jest-e2e.json attendance hr-attendance

# 8. Full gate sweep
pnpm typecheck && pnpm lint && pnpm test && pnpm --filter @munaxa/api build
```
