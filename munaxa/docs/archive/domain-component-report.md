...
New folder **`apps/admin/src/components/domain/`** with a barrel `index.ts`. These compose `@munaxa/ui` primitives and depend on app enums/i18n, so they correctly live in the app (not the generic `@munaxa/ui`), each owning the **single source of truth** for its domain's status colours.
