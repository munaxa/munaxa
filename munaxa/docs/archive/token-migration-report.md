...
Per the Phase-0 governance decision *"`@munaxa/config-tailwind` stays canonical; extend it with missing scales from reference values,"* this phase:
...
| *(verification)* | `pnpm --filter @munaxa/admin build` — ✅ compiles, lint guardrail passes |
...
- ✅ `pnpm --filter @munaxa/admin build` — all 29 routes compile
