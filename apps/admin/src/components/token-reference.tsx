...
const GROUPS: Group[] = [
  {
    ...
    tokens: [
      ...
      { varName: '--coral', use: 'text-coral · bg-coral', kind: 'fill', cls: 'bg-coral' },
      { varName: '--aqua', use: 'text-aqua · bg-aqua', kind: 'fill', cls: 'bg-aqua' },
...
function useLiveVars(names: string[]): Record<string, string> {
  ...
  useEffect(() => {
    ...
    // names is a stable module-level list; intentionally read once on mount.
...
export function TokenReference() {
  ...
  const vals = useLiveVars(allNames);
