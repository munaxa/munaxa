...
/**
 * Contact form schema. Shared between the client form and the `/api/contact` route handler
 * so validation rules never drift between the two.
 *
 * Security notes:
 *  - All free-text fields are trimmed and length-capped to bound storage/email payload size.
 *  - `website` is a honeypot: real visitors never see or fill this field (hidden via CSS).
 *    Bots that auto-fill every field will populate it; the API silently discards those
 *    submissions without revealing that detection occurred.
 *  - `csrfToken` must match the `csrf_token` cookie issued by middleware (double-submit).
 */
export const contactFormSchema = z.object({
  ...
  csrfToken: z.string().min(1, 'Missing security token. Please reload and try again.'),
