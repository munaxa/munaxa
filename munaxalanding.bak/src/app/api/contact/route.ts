...
export async function POST(request: NextRequest) {
  ...
  if (!(await isValidCsrfToken(data.csrfToken))) {
    logger.warn('contact.csrf_failed', { ip: maskIp(ip) });
    return NextResponse.json(
      { ok: false, error: 'Your session has expired. Please reload the page and try again.' },
      { status: 403 },
    );
  }

  // 4. Honeypot — silently accept so bots don't learn they were detected.
  if (data.website) {
    logger.warn('contact.honeypot_triggered', { ip: maskIp(ip) });
    return NextResponse.json({ ok: true });
  }

  // 5. Optional CAPTCHA (Cloudflare Turnstile) — only enforced if configured.
  const captchaOk = await verifyTurnstile(data.turnstileToken, ip);
  if (!captchaOk) {
    logger.warn('contact.captcha_failed', { ip: maskIp(ip) });
    return NextResponse.json(
