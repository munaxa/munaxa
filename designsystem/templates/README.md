...
- Same rules as `components/` and `patterns/`: no product terminology, no business logic, no
  data fetching, no routing library.
- Everything variable arrives through props or slots (`ReactNode`), including every string.
- Compose from `components/` and `patterns/`; a template never re-implements a primitive.
- Export it from `templates/index.ts` and re-export from the package root barrel.
